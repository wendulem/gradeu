...
class MongoOperations {
   ...
   // Queries the database based on user input and returns additional
   // user data
   // based on the input
   Document loginDetails(MongoCollection<Document> collection,
         DBObject obj) {
      Document userdocument = collection.find((Bson) obj)
            .projection(Projections.fields(
                  Projections.include("role", "firstname", "fullname"),
   ...
   List<Integer> getSubjectWeights(
      ...
      // TRY AND MAKE THIS SO IT GETS ALL VALUES OF THIS TYPE WITHOUT
      // SPECIFICATION
   ...
   // Gets the scoregot or scoreout depending on which is specified and
   // what
   // collection it is
   List<Double> calcScoreDoubles(MongoCollection<Document> collection,
         String querytype) {
      List<Double> doubledata = new ArrayList<>();
      collection.find().projection(Projections.include(querytype))
   ...
   List<String> calcScoreStrings(MongoCollection<Document> collection,
         String querytype) {
   ...
   FindIterable<Document> studentFullUser(
         MongoCollection<Document> profilecollection) {
      FindIterable<Document> studentnames = profilecollection
            .find(eq("role", "student"))
