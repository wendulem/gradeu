...
class GradeCalc implements ConstantInterface {
	...
	GradeCalc(String subject, GradeMain grademain, GradeBack gradebackreference) {
		...
		valid_parts = new ArrayList<>(valid_parts_static);
	}
	...
	void calculateNeeded() {
		...
		List<String> scoretype = mongoscoredata.calcScoreStrings(call_collection, "scoretype");
		score_name = mongoscoredata.calcScoreStrings(call_collection, "scorename");

		// Instance to handle all the various calculations needed for the super values
		// and weighting
		GradeTypeCalc typecalc = new GradeTypeCalc(scoretype, call_collection, valid_parts, type_strings);
		// Array of the nonweighted supers for the current subject to be used made final
		List<Double> supersnw = new ArrayList<>();
		for (int j = 0; j < type_strings.size(); j++) {
			double genericsuper = typecalc.processCalc(type_strings.get(j));
			supersnw.add(genericsuper);
		}
		// This is all the data required to properly weight each gradetype for a subject
		int subvalue = typecalc.getSubValue();
		valid_parts = typecalc.getValidParts();

		// This carries out the weight calculations and makes occupies a hashmap with
		// the weights and type
		int scalevalue = 100 - subvalue;
		Map<Integer, Double> scalemap = new HashMap<>();
		for (int i = 0; i < valid_parts.size(); i++) {
			int incrementparts = valid_parts.get(i);
			double incrementalter = incrementparts * (100.0 / scalevalue);
			scalemap.put(incrementparts, incrementalter);
		}
		// This uses the weights and sends the weighted supers to be finalized
		List<Double> supersw = new ArrayList<>();
		for (int k = 0; k < supersnw.size(); k++) {
			Integer validpartsindex = valid_parts.indexOf(valid_parts_static.get(k));
			if (validpartsindex == -1) {
				continue;
			}
			supersw.add(supersnw.get(k) * scalemap.get(valid_parts.get(validpartsindex)));
		}
		finalCalcs(supersw);
	}
	...
	private void finalCalcs(List<Double> supersw) {
		...
		for (int i = 0; i < supersw.size(); i++) {
			if (supersw.get(i) != supersw.get(i)) {
				supersw.set(i, 0.0);
			}
		}
		// Gets the subject super based on these various supers and rounds it off
...
//This method is used within the class but also in another one to make the logout listener hence the if
	...
	class SubjectActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent ae) {
			...
			new GradeStudent(call_collection, score_name, grade_main, gradeback_reference);
