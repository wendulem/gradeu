...
class StudentPanel extends JPanel {
	...
	StudentPanel(int check, backActionListener backaction, MongoCollection<Document> collection) {
		// Below is all the score data that is used to occupy the area from each view
		// button
	...
	private void gradesFill() {
		List<Double> gottype = mongo_type_data.calcScoreDoubles(call_collection, "scoregot");
		List<Double> outtype = mongo_type_data.calcScoreDoubles(call_collection, "scoreout");
		List<String> nametype = mongo_type_data.calcScoreStrings(call_collection, "scorename");
		...
		for (int i = 0; i < check_value; i++) {
			...
			score_labels[i] = new JLabel(gottype.get(i) + "/" + outtype.get(i));
			gbc_field.gridy = 3 + i;
