...
class GradeTeacherPanel extends JPanel implements ConstantInterface {
	...
	GradeTeacherPanel(List<ActionListener> actionlist, JComboBox<String> studentdrop) {
		...
		gbc.weightx = 0.5;
		gbc.weighty = 0.5;
		// Adds the student combobox that was passed as a arugument to reduce data
		// arguments
		this.add(studentdrop, gbc);

		field_array = new ArrayList<>();
		List<String> tooltext = new ArrayList<>();
		// Occupies as tool tip list to be used in an indexing loop with the different
		// boxes they're used with
		tooltext.add("Enter the name of the assignment.");
		tooltext.add("Enter the points the student received for the assignment.");
		tooltext.add("Enter the total points available for the assignment.");
		tooltext.add("Enter the type of grade the assignment is," + " in the format of either: quiz,hw,classwork,test.");
		for (int i = 0; i < 4; i++) {
			JTextField field = new JTextField(10);
			field_array.add(field);
		}
		// Makes the various fields from the field array specific and sets their tool
		// tips
		for (int i = 0; i < 4; i++) {
			gbc.gridx = 2 + i;
			gbc.gridy = 7;
			...
			field_array.get(i).setMinimumSize(field_array.get(i).getPreferredSize());
		}
		// Sets the location and action listeners for the logout and add specialty
		// buttons
		...
		// Sets the locations and actionlisteners for buttons of each of the subjects
	...
	// Gets the buttons and boxes to be used in operations such as the .onClick() to
	// refresh grades
