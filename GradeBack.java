...
class GradeBack implements ConstantInterface {
	...
	GradeBack(GradeMain grademain) {
		// Making the object a field so that it can be passed later and not make another
		// instance necessary
		...
		for (int i = 0; i < length; i++) {
			...
			MongoCollection<Document> collection = DATABASE.getCollection(currentsubject);
			...
			GradeCalc genericcalc = new GradeCalc(currentsubject, grademain, gradeback_reference);
			genericcalc.calculateNeeded();
			// Takes the subject and make it a presentable string for the JLabels
			String usedsubject = genericcalc.getUsedSubject() + ":";
			String modifysubject = usedsubject.substring(0, 1).toUpperCase() + usedsubject.substring(1).toLowerCase();
			subject_masters.add(modifysubject + " " + genericcalc.getSubjectSuper());
