...
/*
 * 
 * Takes user input and authenticates it to instantiate other classes and show the right JFrames and data
 * 
 */
...
class GradeMain implements ConstantInterface {
	...
	private GradeMain() {
		// Establishing MongoClient Connection and getting down to the authentication
		// collection
		mongo_setup = new MongoOperations();
		profile_collection = DATABASE.getCollection("profiles");
		grade_main = this;
		// Creating new instance of class that extends JPanel for GradeMain and doing
		// swing operations
	...
	void operateMain(GradeMain grademain) {
		...
		// Querying Mongo to authenticate the login based off user credentials
		Document userdocument = mongo_setup.loginDetails(profile_collection, obj);
		String userrole = null;
		try {
			// If the input is valid it will check here to get the values and process them
			userrole = userdocument.getString("role");
			name_of_user = userdocument.getString("firstname");
		} catch (NullPointerException e) {
			// Catches null pointer for an invalid input and prompts you to retry
			showMessageDialog(null, "Please Enter a Valid Username or Password!");
		}
		// Checks here to process the user based on their role in the database
